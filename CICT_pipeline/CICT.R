...
# Causal Inference Using Composition of Transactions/Transitions CICT
...
# Using CICT concept and code for gene regulatory network inference is   
# allowed under Attribution-NonCommercial-ShareAlike: CC BY-NC-SA license
################################################################################@
...
if(exists('url.logfile'))
  if(url.logfile!="noLog"){
    write(paste0('===== ====== CICT analysis, Abbas Shojaee, Now: ', lubridate::now(), '======= '),  file = url.logfile, append = TRUE)
...
prepareEdgeFeatures<-function(Debug = F,
  ...
  dt.vertices=setDT(dt.vertices)[,`:=`(OcrInp=ocr,OcrOut=ocr)][,ocr:=NULL]
  ...
  setDT(dt.vertices);dt.vertices[,c('probInp','probOut'):=list(OcrInp/vertexOcrSumIn,OcrOut/vertexOcrSumOut) ] #probability
  ...
  dt.vertices.trgtsum = dt.vertices%>% left_join(dt.edge,by=c("vID"="src")) %>%
  ...
  setDT(dt.vertices);dt.vertices[is.na(SumOcrOut.x),SumOcrOut.x:=1];dt.vertices[is.na(SumOcrInp.y),SumOcrInp.y:=1]
  ...
  dt.edge = replace_na(dt.edge,replace=list(SumOcrOut.x.x=0,SumOcrOut.x.y=0,
  ...
  emissionRate = 1
  maxItmstOcr = max(c(dt.vertices$OcrInp, dt.vertices$OcrOut))
  
  
  dt.edge$OcrOut.x = as.numeric(dt.edge$OcrOut.x )
  dt.edge$OcrInp.y = as.numeric(dt.edge$OcrInp.y )
  
  dt.edge = setDF(dt.edge) %>% dplyr::mutate(conf=Weight/ OcrOut.x , contrib=Weight/OcrInp.y) 
  ...
  #d=setDT(dt.edge)[is.infinite(contribZ),.(tz,odds,contribZ,OcrOut.x,OcrInp.y,I,IBA,DiagnosesAb.x,DiagnosesAb.y)];View(d)
  ...
  if(FALSE)
  {
    ...
    tmp.e=dt.edge %>% dplyr::select(src,trgt,OcrOut.x,OcrInp.y,Weight,SumOcrOut.x.x,SumOcrInp.y.x,Weight,V,R,EO,EI,OEER,OERR,AM,GM,HM,UR)
    ...
    #summary(dt.edge$SumOcrInp.y.x);summary(dt.edge$R);summary(dt.edge$ER);summary(dt.edge$RA)
  ...
  if(TRUE){ 
    setDF(dt.edge)
    #dt.edge<-subset(dt.edge,select=-c(confBA,contribBA,confBA1,contribBA1))
    gc()
    dt.edge.back=dt.edge
    #dt.edge1= dt.edge %>% dplyr::select(src,trgt,IBA=Weight,URBA=UR,AMBA=AM,OEERBA=OEER,OERRBA=OERR,
    ...
    #tmp=dt.edge%>% dplyr::filter(confBA==0) %>% group_by(DiagnosesAb.y) %>% summarize(n=n())
    a=sapply(dt.edge2, function(x) sum(is.na(x)));a[a>0]
    b=sapply(dt.edge2, function(x) sum(x==0 & !is.na(x)));b[b>0]
    
    #dt.edge2$IBA = ifelse(is.na(dt.edge2$IBA),PSUDEO_ZERO,dt.edge2$IBA)
    ...
    #dt.edge2$toeBA <-ifelse(dt.edge2$toeBA==0,toeBAreplacement,dt.edge2$toeBA)
    rm(dt.edge);gc()
  ...
  if(FALSE){ 
    vn=nrow(dt.vertices) ; print(paste0("Total possible edges: ",vn/2*(vn-1)))
    sapply(dt.edge, function(x) sum(is.na(x)))
    
    ggplot(dt.edge, aes(confNDiff))+ggtitle("confNDiff") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(contribN))+ggtitle("contribN") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(confN))+ggtitle("confN") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(contribN))+ggtitle("contribN") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(PDiff))+ggtitle("PDiff") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(Directional))+ggtitle("Directional") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(AMdiff))+ggtitle("AMdiff") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(URdiff))+ggtitle("URdiff") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge, aes(tdiff))+ggtitle("tdiff") + geom_histogram() + scale_x_log10()
    ggplot(dt.edge[!is.na(dt.edge$tRatio),], aes(tRatio))+ggtitle("tRatio") + geom_histogram() + scale_x_log10()
    summary(dt.edge$AMdiff);summary(dt.edge$URdiff);summary(dt.edge$deltaAttitude)
    
    summary(dt.edge$conf)
    
    ggplot2.histogram(data=dt.edge$conf[dt.edge$conf<.002],
    ...
    summary(dt.edge$confN)
    ggplot2.histogram(data=dt.edge$conf[dt.edge$confN<.002],
                      legendPosition="top", xName='', #groupName='',
                      alpha=0.5, fill="cyan",addDensity=TRUE,
                      addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5)
    
    summary(dt.edge$conf)
    histogram(dt.edge$conf[dt.edge$conf<.005])
  }
  #View(dt.edge[str_detect(shared_name,"2860") & str_detect(shared_name,"209"),.(shared_name,causal2,causal11)])
  #View(dt.edge[,c("shared_name","OcrOut.x","OcrInp.y","Weight","conf","contribBA","confBA","contrib")])
  # View(dt.edge[dt.edge$src  %in% c(12676,9355) & dt.edge$trgt %in% c(12676,9355),c("shared_name","OcrOut.x","OcrInp.y","Weight","conf","contribBA","confBA","contrib")])
  ...
  #tmp.e=dt.edge  %>%  select(SUID,shared_name,Weight,src,trgt,OcrOut.x,OcrInp.y,conf,contrib)#%>% filter(src!=trgt)
  ...
  #a=sapply(dt.edge, function(x) sum(is.na(x)));a[a>0]
  #c=sapply(dt.edge, function(x) sum(is.infinite(x)));c[c>0]
  #b=dt.edge[src=='0166D',.(src,trgt,Weight,OcrOut.x,OcrInp.y,Pin,Pinsum.x)];#View(b)
  ...
  #                 PinN=myMax(dt.edge$PinN ),PoutN=myMax(dt.edge$PoutN ),PDiff=myMax(dt.edge$PDiff ),
  #                 PAdd=myMax(dt.edge$PAdd ),PdirDiff=myMax(dt.edge$PdirDiff ),PdirRatio=myMax(dt.edge$PdirRatio ) )
  ...
  if(url.logfile!="noLog") {
    write('Step5.5',  file = url.logfile, append = TRUE)
    write("#########################################################################################@
      # © 2016 Abbas Shojaee <abbas.shojaee@gmail.com> 
      # Causal Inference Using Composition of Transactions/Transitions CICT
      # All rights reserved, please do not use, modify or distribute without written permission
      ...
      # Causal Inference Using Composition of Transactions CICT
      # All rights reserved, please do not use or distribute without written permission
      # Using CICT concept and code for gene regulatory network inference is allowed under 
      # under Attribution-NonCommercial-ShareAlike: CC BY-NC-SA license
      ################################################################################",
      file = url.logfile, append = TRUE)
    }
