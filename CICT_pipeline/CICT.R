...
###########################################################################
#        CICT Network Inference
#        Abbas Shojaee, 2016-2022
#        All rights reserved. Please do not use, reuse or distribute in any form
#        The code is provided only for use at Huang Lab for GRN Analysis at NYU http
###########################################################################
...
n.itm.e = n.itm.e %>%dplyr::mutate(srctrgtSum = OcrOut.x+OcrInp.y,
  ...
  mutate(HTR=(Weight*srctrgtSum)/srctrgtSum,
         EE=OcrOut.x^2*OcrInp.y/srctrgtProduct)#Harmonized transition rate
...
selfContribs=n.itm.e %>% group_by(src) %>% summarise(
  ...
  MADconst=ifelse(is.na(sn::qsc(.65,Mean,SD,Skew)),1.288,sn::qsc(.65,Mean,SD,Skew)), #myQSnorm
  ...
  NMADconst=ifelse(is.na(sn::qsc(.65,NMean,NSD,NSkew)),1.288,sn::qsc(.65,NMean,NSD,NSkew)),
...
selfConf=n.itm.e %>% group_by(src) %>%  summarise(
  ...
  tnMADconst=ifelse(is.na(sn::qsc(.65,tnMean,tnSD,tnSkew)),1.288,sn::qsc(.65,tnMean,tnSD,tnSkew)),
...
othersConfs=n.itm.e %>% group_by(trgt) %>% summarise(
  ...
  NSkew=mySkewness(confN, 0,na.rm =TRUE),
  NKurt=myKurtosis(confN, 3,na.rm = TRUE),
  NMADconst=ifelse(is.na(sn::qsc(.65,NMean,NSD,NSkew)),1.288,sn::qsc(.65,Mean,SD,Skew)),
  NMAD= myMedianAbsoluteDeviation(confN,NMedian,NMADconst,na.rm=TRUE,default=0))
...
n.itm.v=my_replace_na(n.itm.v,rplist = list(Mean=0,Median=0,Skew=0,Kurt=3,
                                            ...
                                            MADcost=1.288,L1=0,L2=0,tau3=0,tau4=0 )) #tau4=0.1226
...
L2 = n.itm.v %>% ungroup() %>% summarise(
  ...
  MADconstOcfSkew=sn::qsc(.65,MeanOcfSkew,SDOcfSkew,SkewOcfSkew), #myQSnorm
  ...
  MADconstScbNSkew=sn::qsc(.65,MeanScbNSkew,SDScbNSkew,SkewScbNSkew), #myQSnorm
  ...
  MADconstOcfNMedian=sn::qsc(.65,MeanOcfNMedian,SDOcfNMedian,SkewOcfNMedian), #myQSnorm
  ...
  MADconstPoutsum=sn::qsc(.65,MeanPoutsum,SDPoutsum,SkewPoutsum), #myQSnorm
  ...
  MADconstScbL3=sn::qsc(.65,MeanScbL3,SDScbL3,SkewScbL3), #myQSnorm
...
n.itm.v=my_replace_na(n.itm.v,rplist = list(Mean=0,Median=0,Skew=0,Kurt=3,MAD=0,
                                            ...
                                            MADcost=1.288,L1=0,L2=0,L3=0,L4=0,tau3=0,tau4=0 ))
...
#Testing performance
{
  ...
  rcrd$unseensmpl_EPR = as.data.frame(reportAUC(sscurves.part) )
  ...
  rcrd$unseensmpl_rndm_classifier = as.data.frame(reportAUC(rndmClscurves.part) )
...
#final run on all edges ------
{
  ...
  rcrd$rankededges_gated_count = nrow(pred_outcome)
