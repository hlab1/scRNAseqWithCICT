...
# install.packages(c("caret", "Rtsne", "umap", "devtools", "hutils", "PerformanceAnalytics", "sn",
#                    "tidyr", "infotheo", "mpmi", "stringr", "RCy3", "igraph", "data.table", "plyr",
#                    "tidyverse", "dplyr", "fitdistrplus", "gendist", "ggplot2", "SimilarityMeasures",
#                    "Lmoments", "sna", "tidygraph", "graphlayouts", "statnet", "knitr","yaml", 'h2o','zeallot'))
...
url.base = "E:/HDDs/Work/NYU Research/0 Plant biology/R code/"
url.CICT_algo="E:/HDDs/Work/NYU Research/0 Plant biology/R code/CICT package - Biology/"
setwd(url.base)

#source all libraries and functions and set up working director
source('E:/HDDs/Work/NYU Research/0 Plant biology/R code/CICT package - Biology/requirements/CICT_LibsFunctions.R')
...
#args.cmnd = c('runCICT','config_L0.yaml','TRUE') #runCICT_par  #runCICT_par  config_SERGIO_DS4.yaml
#args.cmnd = c('calcEdges','config_L0.yaml','TRUE') 
...
if(is.na(arg.experiment)) arg.experiment=''

inputslocation ="Data-Beeline" # "inputs_beeline2" # "inputs"
configFolder = 'config-files_v2/'
...
#CICT caller definition
{
  ...
  CICT<-function(theJobID,
                 ...
                 url.logfile='',
                 url.rankedEdges = NA,
                 url.rankedEdgesGated= NA,
                 url.CICT_algo=NA,
                 cictRawEdgeCol = "Pearson",
                 earlyThresholdForGraphAnalysis = 0,
                 minGroundTruth.ratio.learning = 0.2,
                 maxunseenTest.ratio = 0.1,
                 maxGroundTruth = 250,
                 randomEdgesFoldCausal=5,
                 sampling.c_rc.ratio = 0.3,
                 ...
                 ...)
  {
    ...
    if(is.na(url.CICT_algo)| !dir.exists(url.CICT_algo)) stop("CICT algorithm folder is NA or does not exist")  
    ...
    if(is.na(url.rankedEdges))  url.rankedEdges = paste0(url.output ,'/CICT/',theJobID.tmp, 'rankedEdges.csv')
    if(is.na(url.rankedEdgesGated))  url.rankedEdgesGated = paste0(url.output ,'/CICT/',theJobID.tmp, 'rankedEdgesGated.csv')
    #if(is.na(url.logfile)) url.logfile = paste0(url.output , '/',arg.dname,'_cict_log.txt')
    ...
    if( (!file.exists(url.rankedEdges) | !file.exists(url.rankedEdgesGated)) | forceOutput) {
      ...
      source(paste0(url.CICT_algo,'/CICT1.R'),local=TRUE)
      

      browser()
      c(rcrd,edges,vertices) %<-z% 
        prepareEdgeFeatures(Debug=Debug,
                            cictRawEdgeCol =cictRawEdgeCol,
                            dt.edge = n.itm.e,
                            dt.geneexp = all.dt,
...

{
  ...
  #' @param url.CICT_algo the path to CICT algorithm folder. 
  #' @param outputToCICTsubfolder if True creates a 'CICT' folder under url.input to store intermediary CICT data files including rankedEdges and log file . 
  ...
  #' @param configFilePath Althernative to providing all arguments, you can provide the path to YAML config file containing the required information about input file and desired edge types. Please see example config files  
  #' @param arg.experimentSeries Adds a subfolder under CICT output folder and stores the results. Use it to store results of different parametrization of the function
  ...
  #' @param url.name.map 
  #' @param recalcSimMatrices if TRUE overwrites existing raw edge calculations
  #' @param url.logfile path to log file
                       ...
                       )
  {
    ...
    dir.create(url.inputFolder,recursive = T)
    #Adds a subfolder to CICT for outputs of an experiemntal run, e.g. different params
    ...
    url.RawEdgeCalculationModule=paste0(url.CICT_algo,"requirements/calculateRawEdges.R")
    ...
    if(length(arg.edgeTypes.abs) > 0 ){
      browser()
      similarityMatrices.new = calculateRawEdges(arg.edgeTypes.abs,url.input,n.workers  =n.parallel.workers,url.CICT_algo=url.CICT_algo)
...
if(operation=='config_par'){
  ...
  if(operation=='calcEdges_par'){
      ...
      if(operation=='runCICT' | operation=='calcEdges' | operation=='runSupervised')
    {
      ...
      cnfyaml = read_yaml(paste0(url.base,configFolder,configFilePath)) #configFilePath='config_L2.yaml'
      ...
      { #arg.dname in databases){ #
        ...
        url.inputbase = paste0(url.base,inputslocation,'/',arg.dataFolder)
        url.inputFolder = paste0(url.inputbase ,'/',arg.dname) #,'/CICT')
        
        url.input = paste0(url.inputFolder,'/' , arg.dataFolder,"-",arg.inFile) #arg.dataFolder,'_',
        ...
        url.output = paste0(url.base,'outputs/',  arg.dataFolder,'/',arg.dname) # arg.dataFolder,'/',arg.dname,'/CICT' )
        dir.create(url.output,recursive = T)
        ...
        try({
          ...
          if(operation=='runCICT'){
            ...
            if(!dir.exists(url.output)) dir.create(url.output,recursive = T)
            url.logfile = paste0(url.output , '/',arg.dname,'_cict_log.txt')
            write('Start',  file = url.logfile, append = F)
