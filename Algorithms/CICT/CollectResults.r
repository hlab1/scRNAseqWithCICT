...

{
  ...
  analysisFolder =  "sens_multipleRuns" #'sens_modeling_choices'  #'sens_edgeType' #'sens_sparsity'   #   'sens_learningTrgtAndSize' #
    ...
    tryCatch({
      ...
      if(length(cnf)>1){
        cnf.d=data.frame(  evaluation = cnf$experiment,
                           ...
                           dataset.dir = cnf$dataset_dir,
                           dataset = ifelse(is.null(cnf$datasets[[1]]$name),'',cnf$datasets[[1]]$name),
                           ...
                           trainingTarget = ifelse(is.null(cnf$trainingTrgt ), 'class2' , cnf$trainingTrgt) ,
...
if(F){
  ...
  dt = r.final %>% 
    mutate(subgrps = as.character(edgeType)) %>% #size.groundTruth
    group_by(subgrps)  %>%
    mutate(outcome = unsn_pPRC, outcome.se = sd(unsn_pPRC,na.rm=T) / sqrt(nrow(r.final))) %>%
     
    summarize(est = mean(outcome),
