...
cictTrainTestReport <-function(n.itm.e,rcrd,method='CICT', 
                               evaluateUnseenSample = T, evaluateAllEdges = F,
                               supervisedFeatures = c(),Debug = F,
  ...
  
  {
    ...
    #if(NcausalEdges<300) floor(2*nrow(t1.c)*minGroundTruth.ratio.learning)
    if(NcausalEdges<45){
      write(sprintf("ATTENTION! Number of causal edges for learning set was %s, and increased to minimum required: 50 ",NcausalEdges),  file = url.logfile, append = TRUE)
      NcausalEdges=45
    ...
    msg=sprintf(' Total true edges in ground truth= %s, Total true edges in learning set = %s , \n Network density= %s, learning set density= %s, \n  total edges= %s \n learning set=%s, test fraction = %s , \n learning set random= %s, learning set causal edges= %s',  
                nrow(tbl.goldStandard), sum(t2$predicate=='CAUSES'),
  ...
  if(F){
    ...
    t1.causal_reversecausal=rbind(t1.c,t1.rc)
    t1.causal_reversecausal.sub = 
      if(t1.gtfrac==1) t1.causal_reversecausal else 
        sample_frac(t1.causal_reversecausal, size = t1.gtfrac)  
  ...
  if(FLAG_exportTrainAndTest & (length(preset.train)==1 && is.na(preset.train)) & !usepresettraintest){
    try({
      ...
      d.learning = rbind(tst1.train,tst1.tst) %>% mutate(learningset = 1)
      
      
      if(nrow(d.learning[class2==T,]) > 250) browser()
  ...
  if(Debug==5) saveRDS(list(rcrd,prd.varimp,n.itm.e,pred_outcome.e,t1),file = paste0(url.output , '/CICT network objects.rds'))
