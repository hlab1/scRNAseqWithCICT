...
write('Step 1',  file = url.logfile, append = TRUE)
...
write('Step5',  file = url.logfile, append = TRUE)
...
setDF(n.itm.e); setDF(n.itm.v1)
write('Step5.1',  file = url.logfile, append = TRUE)
...
dupcols = removeDups1(n.itm.e,excluded = c('OcrInp.x','Indegree.x'), samplesize = 20000)
...
if(F){
  n.itm.e= n.itm.e %>%  
  dplyr::mutate(
    sconfZ.x=(conf-scfMedian.x)/ifelse(scfMAD.x==0,PSUDEO_ZERO_2,scfMAD.x), #Zscore based on median and MAD
    scontribZ.x=(contrib-scbMedian.x)/ifelse( scbMAD.x==0,PSUDEO_ZERO_2 ,scbMAD.x ) ,
    ##oconfZ.x=(conf-ocfMedian.x)/ifelse(ocfMAD.x==0,PSUDEO_ZERO_2,ocfMAD.x), #s23c0l-
    ##ocontribZ.x=(contrib-ocbMedian.x)/ifelse( ocbMAD.x ==0,PSUDEO_ZERO_2 ,ocbMAD.x ) , #s23c0l-
    
    sconfZ.y=(conf-scfMedian.y)/ifelse(scfMAD.y==0,PSUDEO_ZERO_2,scfMAD.y), #Zscore based on median and MAD
    scontribZ.y=(contrib-scbMedian.y)/ifelse( scbMAD.y==0,PSUDEO_ZERO_2 ,scbMAD.y ) 
    ##oconfZ.y=(conf-ocfMedian.y)/ifelse(ocfMAD.y==0,PSUDEO_ZERO_2,ocfMAD.y),
    ##ocontribZ.y=(contrib-ocbMedian.y)/ifelse( ocbMAD.y ==0,PSUDEO_ZERO_2 ,ocbMAD.y )  
  )
  }
...
#Testing performance
{
  ...
  msg = sprintf("AUCPR Ratio CICT to Random= %s,  Partial AUCPR Ratio CICT to Random= %s ", 
