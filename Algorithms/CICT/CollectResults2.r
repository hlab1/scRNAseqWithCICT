...
if(F){
  #Extract results 
  {
    thepath='/scratch/as15096/eric/outputs/'
    f.all = list.files(path = thepath,pattern = '_rslt_.*[.]rds', recursive = T)
    r.all =  data.frame()
    for(r.file in f.all){
      #print(r.file)
      ...
      try({
        ...
        r = readRDS(paste0(thepath, r.file))
        level_data =  r.file %>% dirname() %>% dirname()
        if(length(r)>1){
          print(r.file)
          ...
          r.flat =r.flat %>% as.data.frame() %>% 
            mutate(#job_idx = str_extract(r.file,pattern = '(?<=[_]).*(?=[.]rds)',) %>%as.integer(),
                   benchmark = level_data %>% dirname() %>% basename(),
                   dataset = level_data %>% basename()) 
    ...
    r.all =r.all %>% dplyr::select(`benchmark`, dataset,matches("unsn.*"),matches("rndm.*"), everything())
    write.csv(r.all,file=paste0(thepath,"collectedResults.csv"))
  }
...
#Report on ground truth usage
{
  reportTFinLearningSet<-function(dataFolder,db,algo, url.refnet,url.output ,url.logfile)
  {
    print(paste0("Doing ", dataFolder," " ,db, "==============="))
    evals = data.frame()
    
    # url.refnet = paste0(url.input,dataFolder,'/',db,'/refNetwork.csv' )
    d.ref = fread(url.refnet) %>% mutate(outcomes = 1)
    table(d.ref$outcomes)
    write(paste0("Colnames dref:" , colnames(d.ref) %>% paste0(collapse=',')),  file = url.logfile, append = TRUE)
    
    url.train = paste0(url.output,dataFolder,'/',db, '/CICT','/train.csv' )
    d.train = fread(url.train) %>%  mutate(testset=0)
    
    
    url.tst = paste0(url.output,dataFolder,'/',db, '/CICT','/test.csv' )
    d.test= fread(url.tst) %>%  mutate(testset=1)
    
    d.learning = rbind(d.train,d.test) %>% mutate(learningset = 1)
    
    
    
    d.l = d.learning %>% unique()%>% left_join(d.ref, by = c('Gene1', 'Gene2')) %>%
      mutate(outcomes = ifelse(is.na(outcomes),0,outcomes),
             testset = ifelse(is.na(testset),0,testset),
             learningset = ifelse(is.na(learningset),0,learningset)) %>%
      filter(outcomes==1)
    
    # write("Table learningset * outcome ============",  file = url.logfile, append = TRUE)
    # table(d.l$learningset,d.l$outcomes)
    # write(table(d.l$test,d.l$outcomes) %>% knitr::kable(),  file = url.logfile, append = TRUE)
    # 
    # write("Table test * outcome ============",  file = url.logfile, append = TRUE)
    # table(d.l$test,d.l$outcomes)
    # write(table(d.l$test,d.l$outcomes) %>% knitr::kable(),  file = url.logfile, append = TRUE)
    
    
    
    alltfs = d.ref$Gene1 %>% unique()
    
    #table(assespreds$predictions,assespreds$outcomes)
    
    
    msg = sprintf("%s %s | learning rows: %s | d.l rows: %s | Total true edges: %s | True edges in learning set: %s | Learningset/all true edges: %s | Number of TFs= %s | TFs in learning set= %s | Learningset TFS/All= %s", 
                  dataFolder,db,  
                  nrow(d.learning),nrow(d.l),
                  round(nrow(d.ref[outcomes==1 ,]),3),
                  round(nrow(d.l[outcomes==1 & learningset==1,]),3),
                  round(nrow(d.l[outcomes==1 & learningset==1,])/nrow(d.ref[outcomes==1 ,]),3),
                  length(alltfs),
                  round(d.l[outcomes==1 & learningset==1,]$Gene1 %>% unique() %>% length(),3),
                  round(d.l[outcomes==1 & learningset==1,]$Gene1 %>% unique() %>% length()/length(alltfs),3))
    print(msg)
    write(msg,  file = url.logfile, append = TRUE)
    
    if(round(nrow(d.l[outcomes==1 & learningset==1,]),3)>300) return(msg) else return('')
  }
  
  #debug(reportTFinLearningSet)
  url.input =paste0("/scratch/as15096/eric",'/inputs_beeline2/')
  url.output ='/scratch/as15096/eric/outputs_v2/'
  url.logfile  = paste0(url.output, 'CICT_TFs.txt')
  
  
  f.all = list.files(path = '/scratch/as15096/eric/outputs_v2/',pattern = 'train.csv', recursive = T)
  f.allexcept.par = f.all[str_detect(f.all,'cict_par',negate = T)]
  ...
  for(f.r in f.all){
    ...
    path.splt = str_split(str_replace(f.r,'/train.csv',''), '/') %>% unlist() 
    
    db=path.splt[length(path.splt)-1]
    dataFolder = path.splt[1]
    
    if(str_detect(dataFolder,"SERGIO")){
      d.gs=paste0('/scratch/as15096/eric/inputs_beeline2/',dataFolder,'/',db,'/',paste0('refNetwork.csv'))
    }else 
      d.gs=paste0('/scratch/as15096/eric/inputs_beeline2/',dataFolder,'/',db,'/',paste0(dataFolder,'-network.csv'))
    
    f.all.wrongs = c(f.all.wrongs ,reportTFinLearningSet(dataFolder,db,'CICT',d.gs ,url.output ,url.logfile))
...
#Extract and integrate results from batch opration
{
  ...
  #Extract results 
  {
    ...
    path.base = "/scratch/as15096/eric/outputs_v2/"
  ...
  #Batch opration
  {
    ...
    library(doFuture)
    registerDoFuture();
    plan(multisession, split = TRUE, workers = 12)
    ...
    analysisFolder = 'sens_modeling_choices'  #  'sens_sparsity'   # "sens_multipleRuns" # 'sens_edgeType'  #  'sens_learningTrgtAndSize' #
    
    path.base = "/scratch/as15096/eric/outputs_v2/cict_par/"
    r.path = paste0(path.base,analysisFolder,"/")
    
    cnf.files = list.files(r.path,pattern = '.*[.]yaml',recursive = T)
    r.evals = data.frame()
    ...
    try({r.final = r.final %>% mutate(rpAUPR =unsn_pPRC/rndm_pPRC  )})
    glimpse(r.final)
