...
util$last<-function(x){ sort(x)[length(x)] }
...
util$binary_eval <- function(pred,labels, cutoff='naive', repar=TRUE, ...) {
  ...
  rroc = performance(rocr_pred,'tpr','fpr')
  ...
  pp = function(ind,...) points(pr_curve@x.values[[1]][ind], pr_curve@y.values[[1]][ind], ...)
  ...
  warning = function(war){
    print(paste("EVAL WARNING:  ",war));
  },
  error = function(err){
    print(paste("EVAL ERROR:  ",err));
  });
  ...
  scalars <- c(acc@y.values[[1]][cutoff_ind],(if (bac[1] != -1) mean(bac) else NA), f1@y.values[[1]][cutoff_ind], pr_curve@y.values[[1]][cutoff_ind],pr_curve@x.values[[1]][cutoff_ind],roc@x.values[[1]][cutoff_ind],auc,cutoff);
  names(scalars) <- c("ACC","BalACC","F","PREC","REC","SPEC","AUC","CUTOFF")
  #print(scalars);
  invisible(scalars);
}
